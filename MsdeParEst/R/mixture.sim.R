...
# mixture.sim <- function(M, density.phi, param) { propmixed <- param[1] p1 <-
# rbinom(M, 1, propmixed) if (density.phi == 'mixture.normal') { mu1 <- param[2] omega1
# <- param[3] mu2 <- param[4] omega2 <- param[5] Y <- p1 * rnorm(M, mean = mu1, sd =
# omega1) + (1 - p1) * rnorm(M, mean = mu2, sd = omega2) } if (density.phi ==
# 'mixture.gamma') { shape1 <- param[2] scale1 <- param[3] shape2 <- param[4] scale2 <-
# param[5] Y <- p1 * rgamma(M, shape = shape1, scale = scale1) + (1 - p1) * rgamma(M,
# shape = shape2, scale = scale2) } return(Y) }
